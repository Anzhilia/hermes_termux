...
class LocalModelForegroundService : Service() {
    companion object {
        const val CHANNEL_ID = "hermes_local_model"
        ...
        private const val RUNTIME_PID_HOST_RELATIVE_PATH =
            "rootfs/ubuntu/root/.hermes/modules/llama.cpp/runtime/server.pid"
    ...
    private fun buildServerCommand(
        ...
        return listOf(
            "mkdir -p ${shellQuote("/root/.hermes/modules/llama.cpp/logs")} ${shellQuote("/root/.hermes/modules/llama.cpp/runtime")} ${shellQuote("/root/.cache/llama.cpp")}",
            "touch ${shellQuote(logPath)}",
            "printf '%s\\n' \"\$\$\" > ${shellQuote("/root/.hermes/modules/llama.cpp/runtime/server.pid")}",
