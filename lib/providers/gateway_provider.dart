...
class GatewayProvider extends ChangeNotifier with WidgetsBindingObserver {
  ...
  GatewayProvider() {
    ...
    _subscription = _gatewayService.stateStream.listen(
      (state) {
        _state = state;
        notifyListeners();
      },
      onError: (Object error) {
        debugPrint('[GatewayProvider] stream error (non-fatal): $error');
      },
    );
    // Check if gateway is already running (e.g. after app restart)
    // Wrap in error handler — PRoot may not be ready yet after fresh setup,
    // and an unhandled exception here would crash the entire widget tree.
    _gatewayService.init().catchError((Object e) {
      debugPrint('[GatewayProvider] init failed (non-fatal): $e');
