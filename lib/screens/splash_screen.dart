...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _checkAndRoute() async {
    ...
    try {
      ...
      // ★ Fast path: if setup was already completed in a previous session,
      // skip the expensive PRoot-based isBootstrapComplete() check.
      // This prevents the setup wizard from flashing on every app open.
      // Note: gateway config (API model) is NOT required — users can
      // configure it later from the Dashboard Providers screen.
      ...
      try {
        ...
        if (oldVersion != null && oldVersion != AppConstants.version) {
          ...
          if (hasPermission) {
            ...
            final snapshotPath =
                '$sdcard/Download/hermes-snapshot-$oldVersion.json';
            final hermesJson = await NativeBridge.readRootfsFile(
                'root/.hermes/hermes.json');
      ...
      if (setupComplete) {
        // Setup is complete — always go to Dashboard.
        // API model configuration is NOT required; users can configure
        // it later from the Dashboard Providers screen.
