...
class _BackupManagerScreenState extends State<BackupManagerScreen> {
  ...
  Future<void> _refresh() async {
    try {
      ...
      final entries = await BackupLibraryService.listEntries(
        ...
        currentHermesVersion: installedHermesVersion,
      );
      if (!mounted) {
        return;
      }
      setState(() {
        _entries = entries;
        _currentHermesVersion = installedHermesVersion;
        _loading = false;
      });
    } catch (error) {
      if (!mounted) {
  ...
  Future<void> _importExternalBackup() async {
    ...
    try {
      final entry = await BackupLibraryService.importBackupFromPicker(
        currentAppVersion: AppConstants.version,
        currentHermesVersion: _currentHermesVersion,
      );
      if (entry == null) {
        return;
      }
    ...
    try {
      final saved = await BackupService.exportConfigBackup(
        suggestedName: _defaultConfigBackupFileName(
          ...
          hermesVersion: _currentHermesVersion,
        ),
      );
  ...
  String _defaultConfigBackupFileName({
    ...
    final hermesPart = _sanitizeBackupFilePart(hermesVersion);
    return 'hermes-config-app-$appPart-hermes-$hermesPart-${_backupTimestampSuffix()}.json';
  }
  ...
  String _defaultWorkspaceBackupFileName({
    ...
    final appPart = _sanitizeBackupFilePart(appVersion);
    final hermesPart = _sanitizeBackupFilePart(hermesVersion);
    return 'hermes-workspace-app-$appPart-hermes-$hermesPart-${_backupTimestampSuffix()}.zip';
  }
