...
当你想重新走一次 Hermes 的初始化流程，或者之前没有安装 daemon，现在想一起补上时，可以用这个方式。
...
'''),
        _GuideBlock.code(
          ...
          textZh: 'hermes setup --install-daemon',
          textEn: 'hermes setup --install-daemon',
        ),
        _GuideBlock.markdown('''
### 3. 按提示完成引导
...
- 如果你只是想修改局部配置，不一定要重新引导，通常 `hermes config` 就够了。
- 建议在没有重要会话运行时再做这类操作。
'''),
      ],
      blocksEn: [
...
- If you only need to change part of the configuration, `hermes config` is usually enough.
...
'''),
        _GuideBlock.code(
          titleZh: '执行命令',
          titleEn: 'Run this command',
          textZh: 'hermes config set tools.profile full',
          textEn: 'hermes config set tools.profile full',
        ),
        _GuideBlock.markdown('''
### 3. 检查是否生效

如有需要，可以打开 `hermes.json`，确认 `tools.profile` 已经变成 `full`。
...
If needed, open `hermes.json` and confirm that `tools.profile` is now `full`.
...
'''),
    ...
    const _GuideSpec(
      ...
      summaryZh: '重新打开 `hermes config` 的交互式配置流程。',
      summaryEn: 'Open the interactive `hermes config` flow again.',
      blocksZh: [
        _GuideBlock.markdown('''
## 适用场景
...
'''),
        _GuideBlock.code(
          titleZh: '执行命令',
          titleEn: 'Run this command',
          textZh: 'hermes config',
          textEn: 'hermes config',
        ),
        _GuideBlock.markdown('''
### 3. 进入需要修改的配置项
...
用另一台局域网设备访问 Hermes，第一次通常会看到类似下面的报错：
'''),
        _GuideBlock.code(
          titleZh: '常见报错',
...
打开配置文件，在 `controlUi.allowedOrigins` 数组下面添加当前局域网访问地址加 `:${AppConstants.gatewayPort}` 端口，保存后再刷新页面。
'''),
        _GuideBlock.code(
          titleZh: '示例地址',
          titleEn: 'Example origin',
          textZh: 'http://192.168.1.23:${AppConstants.gatewayPort}',
          textEn: 'http://192.168.1.23:${AppConstants.gatewayPort}',
        ),
        _GuideBlock.markdown('''
### 7. 如果浏览器提示需要设备身份
...
'''),
        _GuideBlock.code(
          ...
          textZh: 'hermes pairing list',
          textEn: 'hermes pairing list',
        ),
        _GuideBlock.code(
          titleZh: '批准请求',
          titleEn: 'Approve the request',
          textZh: 'hermes pairing approve <请求ID>',
          textEn: 'hermes pairing approve <request-id>',
        ),
        _GuideBlock.prompt(
          titleZh: '可直接发给 Hermes 的提示词',
          titleEn: 'Prompt you can send to Hermes',
          textZh:
              '请帮我在当前终端完成局域网设备授权：先执行 `hermes pairing list` 找出待审批的请求 ID，再执行 `hermes pairing approve <请求ID>`，最后把执行结果回显给我。',
          textEn:
              'Please help me approve the LAN device from the current terminal: first run `hermes pairing list` to find the pending request ID, then run `hermes pairing approve <request-id>`, and finally show me the command output.',
        ),
        _GuideBlock.markdown('''
## 补充说明
...
Open the Hermes interactive configuration flow from a terminal.
'''),
        _GuideBlock.code(
          titleZh: '执行命令',
          titleEn: 'Run this command',
...
Open Hermes from another device on the same LAN. The first visit usually shows this error:
'''),
        _GuideBlock.code(
          titleZh: '常见报错',
          titleEn: 'Expected first error',
...
Open the config file and add the actual LAN access address plus `:${AppConstants.gatewayPort}` to `controlUi.allowedOrigins`, then save and refresh.
'''),
        _GuideBlock.code(
          titleZh: '示例地址',
          titleEn: 'Example origin',
