...
class _ConfigEditorScreenState extends State<ConfigEditorScreen> {
  ...
  Future<void> _loadConfig() async {
    ...
    try {
      ...
      // Try to convert YAML to pretty-printed JSON for editing.
      // If conversion fails, show raw content as-is.
      _controller.text = _yamlToJsonOrRaw(normalized);
  ...
  /// Convert YAML source to pretty-printed JSON for editing.
  /// Falls back to raw source if parsing fails.
  String _yamlToJsonOrRaw(String source) {
    try {
      final parsed = loadYaml(source);
      if (parsed != null) {
        final json = _yamlToEncodable(parsed);
        return const JsonEncoder.withIndent('  ').convert(json);
      }
    } catch (_) {}
    // Already valid JSON? Just pretty-print.
    try {
      final decoded = jsonDecode(source);
  ...
  /// Recursively convert yaml objects to JSON-encodable types.
  static dynamic _yamlToEncodable(dynamic value) {
    if (value is YamlMap) {
      final map = <String, dynamic>{};
      for (final entry in value.entries) {
        map[entry.key.toString()] = _yamlToEncodable(entry.value);
      }
      return map;
    }
    if (value is YamlList) {
      return value.map(_yamlToEncodable).toList();
    }
    return value;
  }

  String _prettyPrintJson(String source) {
    // Try YAML first (config.yaml files)
    try {
      final parsed = loadYaml(source);
      if (parsed != null) {
        final json = _yamlToEncodable(parsed);
  ...
  /// Convert JSON string back to YAML for saving .yaml config files.
  String _jsonToYamlOrRaw(String source) {
    try {
      final decoded = jsonDecode(source);
      if (decoded is Map<String, dynamic>) {
        return _mapToYamlString(decoded);
      }
    } catch (_) {}
    // If it's already valid YAML (not JSON), save as-is
    try {
      loadYaml(source);
      return source;
    } catch (_) {}
  ...
  /// Convert a Map to YAML string (matching gateway_service.dart approach).
  static String _mapToYamlString(Map<String, dynamic> config, {int indent = 0}) {
    ...
    for (final entry in config.entries) {
      ...
      if (value is Map<String, dynamic>) {
        if (value.isEmpty) {
          ...
          buffer.writeln('$prefix$key:');
          buffer.write(_mapToYamlString(value, indent: indent + 1));
        }
  ...
  Future<void> _saveConfig() async {
    ...
    try {
      // Convert editor JSON back to YAML for .yaml config files
      final formatted = _jsonToYamlOrRaw(_controller.text);
