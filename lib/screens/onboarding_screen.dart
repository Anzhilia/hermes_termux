...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _startOnboarding() async {
    ...
    try {
      ...
      const onboardingCommand = 'hermes setup';
      final commandParts = <String>[
        if (!bonjourEnabled) 'export HERMES_DISABLE_BONJOUR=1',
      ...
      // Replace with [..., '/bin/bash', '-lc', 'hermes setup']
      ...
      _pty!.output.cast<List<int>>().listen((data) {
        ...
        // Scan output for token URL (e.g. http://localhost:${AppConstants.gatewayPort}/#token=...)
        _urlScanBuffer += text;
  ...
  Future<void> _goToDashboard() async {
    // ★ Kill pty BEFORE navigation to prevent it from blocking the new screen.
    _pty?.kill();
    _pty = null;
    // Fire-and-forget: don't await — let navigation proceed immediately.
    NativeBridge.stopTerminalService().catchError((_) {});

    final navigator = Navigator.of(context);

    // Save prefs with timeout — SharedPreferences.getInstance() can hang
    // on some Android devices if called too soon after activity recreation.
    try {
      final prefs = PreferencesService();
    ...
    // ★ Force-navigate to dashboard WITHOUT checking hasRequiredGatewayConfig().
    // User may not have configured an API key yet — that's fine, they can
    // configure later from the dashboard.  Do NOT redirect back to onboarding.
    if (mounted) {
