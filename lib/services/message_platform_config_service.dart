...
/// Reads and writes messaging platform configuration in config.yaml.
class MessagePlatformConfigService {
  ...
  static const qqbotPluginPackage = '@tencent-connect/hermes-qqbot@latest';
  static const qqbotConnectUrl = 'https://q.qq.com';
  static const weixinPluginPackage = '@tencent/hermes-weixin';
  static const weixinInstallerCommand =
      'npx -y @tencent-weixin/hermes-weixin-cli install';

  static String _shellEscape(String s) {
    return "'${s.replaceAll("'", "'\\''")}'";
  }

  static String _wrapHermesCommand(String command) {
    return '''
export HOME=/root
...
''';
  ...
  static Future<String> _runHermesCommand(
    String command, {
    int timeout = 120,
  }) async {
    try {
...
''';
  ...
  static Future<bool> isQqbotPluginInstalled() async {
    try {
      final output = await _runHermesCommand(
        'hermes plugins list',
        timeout: 45,
      );
      final lower = output.toLowerCase();
      return lower.contains('@tencent-connect/hermes-qqbot') ||
  ...
  static Future<bool> isWeixinPluginInstalled() async {
    try {
      ...
      final lower = output.toLowerCase();
      return lower.contains('@tencent/hermes-weixin') ||
          lower.contains('hermes-weixin') ||
  ...
  static Future<void> ensureQqbotPluginInstalled() async {
    ...
    await _runHermesCommand(
      'hermes plugins install $qqbotPluginPackage',
      timeout: 600,
    );
  }
  ...
  static Future<void> configureQqbot({
    ...
    await _runHermesCommand(
      'hermes channels add --channel qqbot --token ${_shellEscape(token)}',
      timeout: 120,
    );
