...
class NodeService {
  ...
  /// 2. config.yaml / .env persisted config (for local gateway)
  ...
  Future<String?> _readGatewayToken() async {
    ...
    if (configuredToken != null && configuredToken.isNotEmpty) {
      _log('[NODE] Gateway token loaded from config.yaml');
    ...
    try {
      ...
      if (code != null) {
        ...
        if (isLocal) {
          ...
          try {
            await NativeBridge.runInProot('hermes pairing approve $code');
  ...
  /// 从 disabled 状态切换，表示节点已启用
  /// Hermes 架构：手机在 18790 端口等待 gateway 侧 bridge 连入
  void updateStateForEnable() {
    if (_state.status == NodeStatus.disabled ||
      ...
      _log('[NODE] Node enabled — listening on :18790');
      _log('[NODE] Waiting for gateway bridge to connect...');
    }
  }

  /// 桥接器已连接（收到 bridge.hello）
  void updateStateForPaired() {
    _updateState(_state.copyWith(
      status: NodeStatus.paired,
      connectedAt: DateTime.now(),
      clearError: true,
    ));
    _log('[NODE] Gateway bridge connected!');
  }
