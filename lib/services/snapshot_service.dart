...
class SnapshotCompatibility {
  const SnapshotCompatibility({
    ...
    required this.snapshotHermesVersion,
    required this.currentHermesVersion,
  });

  final String? snapshotAppVersion;
  final String? currentAppVersion;
  final String? snapshotHermesVersion;
  final String? currentHermesVersion;

  bool get hasMissingVersionInfo =>
      snapshotAppVersion == null ||
      snapshotHermesVersion == null ||
      currentAppVersion == null ||
      currentHermesVersion == null;
  ...
  bool get hasHermesVersionMismatch =>
      _hasComparableVersions(
        snapshotHermesVersion,
        currentHermesVersion,
      ) &&
      !HermesVersionService.isSameVersion(
        installedVersion: snapshotHermesVersion,
        targetVersion: currentHermesVersion,
      );

  bool get requiresConfirmation =>
      hasMissingVersionInfo ||
      hasAppVersionMismatch ||
      hasHermesVersionMismatch;
...
class SnapshotService {
  static Future<Map<String, dynamic>> buildSnapshot({
    ...
    return {
      ...
      'hermesVersion': _normalizeVersion(hermesVersion),
      'timestamp': DateTime.now().toIso8601String(),
      'hermesConfig': hermesJson,
  ...
  static String? snapshotHermesVersion(Map<String, dynamic> snapshot) {
    return _normalizeVersion(snapshot['hermesVersion']);
  }

  static SnapshotCompatibility analyzeCompatibility(
    Map<String, dynamic> snapshot, {
    required String currentAppVersion,
    String? currentHermesVersion,
  }) {
    return SnapshotCompatibility(
      ...
      snapshotHermesVersion: snapshotHermesVersion(snapshot),
      currentHermesVersion: _normalizeVersion(currentHermesVersion),
    );
  }
