...
class _GatewayControlsState extends State<GatewayControls> {
  final _hermesVersionService = HermesVersionService();
  ...
  Future<void> _refreshInstalledVersion({bool showLoading = true}) async {
    ...
    try {
      final version = await _hermesVersionService.readInstalledVersion();
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _loadReleaseOptions() async {
    ...
    try {
      final latestRelease = await _hermesVersionService.fetchLatestRelease();
      List<HermesReleaseInfo> availableReleases;
      Object? releaseOptionsError;
      try {
        availableReleases = await _hermesVersionService.fetchAvailableReleases();
      } catch (e) {
  ...
  Future<void> _installSelectedRelease(GatewayProvider provider) async {
    ...
    if (HermesVersionService.isSameVersion(
  ...
  Widget _buildVersionCard(
    ...
    final installedVersion = _loadingInstalledVersion
        ? '...'
        : _installedVersion ?? l10n.t('dashboardHermesVersionUnknown');
    final latestRelease = _latestRelease;
    final selectedRelease = _selectedRelease ?? latestRelease;
    final isSelectedVersionInstalled = HermesVersionService.isSameVersion(
      installedVersion: _installedVersion,
      targetVersion: selectedRelease?.version,
    );
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      l10n.t('dashboardHermesVersionLabel'),
                      style: theme.textTheme.labelSmall?.copyWith(
                        color: theme.colorScheme.onSurfaceVariant,
                        fontWeight: FontWeight.w600,
                      ),
  ...
  Widget? _buildInstalledVersionStatus(
    ...
    required HermesReleaseInfo? latestRelease,
  }) {
    ...
    if (HermesVersionService.isUpdateAvailable(
