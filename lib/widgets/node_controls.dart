...
class NodeControls extends StatelessWidget {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // 节点已启用但未配对时，显示提示信息
                if (!state.isDisabled && !state.isPaired && state.errorMessage == null) ...[
                  const SizedBox(height: 4),
                  Text(
                    '正在连接到 Hermes Gateway...',
                ...
                Wrap(
                  ...
                  children: [
                    if (state.isDisabled)
                      FilledButton.icon(
                        onPressed: () => _handleEnable(context, provider),
  ...
  Future<void> _handleEnable(
      BuildContext context, NodeProvider provider) async {
    try {
      await provider.enable();
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('启动节点失败: $e')),
        );
      }
    }
  }
